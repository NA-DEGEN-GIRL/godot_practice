...
- **전투 시스템**: 적 클릭 근접 공격, 권총 사격 (우클릭), 크리티컬 히트 (20% 확률, 2배 데미지)
- **스킬 시스템**:
  - ⚡ **번개 (1번키)**: 대상 지정 즉발 스킬, 지그재그 번개 이펙트, 감전 스턴(Enemy3)
  - 🔥 **화염방사기 (2번키)**: 홀드형 지속 스킬, GPU 파티클, 최대 5초
  - 💨 **순간이동 (3번키)**: 마우스 방향 순간이동, 충돌 체크, 카메라 흔들림
- **4종 적 AI**:
  - Enemy: 원거리 독 투사체 (포물선, 회피 가능)
  - Enemy2: 배회/추격/근접 공격 상태 머신
  - Enemy3: 번개에 감전 반응 (3초 스턴 + 감전 애니메이션)
  - Enemy4: 게으른 수면 사이클 + 강력 돌진 공격 (HP 1/3)
- **인벤토리 & 아이템**: 8칸 인벤토리, 권총 장착, 치킨 HP 회복, 탄약 픽업
- **HP/데미지 시스템**: 3D HP 바, 떠오르는 데미지 숫자 (일반/크리티컬 구분)
- **UI**: 스킬바 (쿨타임), HP/SP 게이지, 탄약 표시, 볼륨 조절 슬라이더
- **사운드**: 스킬별 효과음, 적별 고유 사운드, Python scipy 합성
...
| 좌클릭 (아이템) | 아이템 줍기 (가까이에서) |
| 우클릭 | 권총 발사 (장착 시, 마우스 방향) |
...
├── test_scene.gd              # 아이템/적 스폰 및 리스폰 관리
├── player.gd / player.tscn    # 플레이어 (이동, 공격, 스킬, 인벤토리, 사격)
├── enemy.gd / enemy.tscn      # 적1 (독 투사체, 원거리)
├── enemy2.gd / enemy2.tscn    # 적2 (근접, 배회/추격/공격 AI)
├── enemy3.gd / enemy3.tscn    # 적3 (번개 감전 스턴, 약한 근접)
├── enemy4.gd / enemy4.tscn    # 적4 (수면/배회 사이클, 강력 돌진)
├── camera_follow.gd           # 부드러운 카메라 추적
├── skill_bar.gd               # 스킬바 UI + HP/SP 게이지 + 볼륨 조절
├── inventory_ui.gd            # 인벤토리 UI (I키, 장착/버리기)
...
├── sounds/                    # 효과음 (외부 에셋 + Python scipy 합성)
├── models/                    # AI 생성 3D 모델 (.glb)
...
8. **[오디오](docs/08-audio.md)** - 사운드 재생, 루프, 볼륨 조절
9. **[MCP와 AI 에셋 파이프라인](docs/09-mcp-ai-pipeline.md)** - MCP 서버 구성과 AI 기반 에셋 생성
10. **[인벤토리와 아이템](docs/10-inventory-and-items.md)** - 인벤토리, 픽업, 리스폰 시스템
11. **[적 AI 시스템](docs/11-enemy-ai.md)** - 4종 적 AI, 상태 머신, 감전 스턴, 수면 사이클
