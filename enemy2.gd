...
func _detect_animations() -> void:
	...
	for a in anims:
		...
		if "walk" in lower:
			...
			_anim_attack = a
	# Fallback: if missing, use first available
...
func _physics_process(delta: float) -> void:
	...
	var state := "wander"  # wander, chase, attack
...
func _on_animation_finished(anim_name: String) -> void:
	if _is_attacking_anim:
		...
		# Apply damage at end of attack if target still in range
		if _target and is_instance_valid(_target) and not _target.is_dead:
...
func _play_anim(anim_name: String) -> void:
	if not _anim_player or anim_name == "" or _is_attacking_anim:
		return
	# Replay if different anim OR same anim finished (not looping)
